...
async function fetchRuleRows(supabase: QueryableSupabase, outputType: string | null | undefined, platform: string | null | undefined) {
  const [templates, writingRules, assemblyRules, validationRules, platformRules, knowledgeBlocks] = await Promise.all([
    ...
    supabase.from("prompt_writing_rules").select("id, rule_name, rule_category, description, applies_to_output_types, applies_to_platforms, priority, example_good, example_bad, repair_instruction").eq("active", true).order("priority", { ascending: false }).limit(20),
    ...
    supabase.from("platform_rules").select("id, platform_name, platform_type, prompt_syntax, supports_negative_prompt, aspect_ratio_rules, recommended_prompt_length, parameter_rules, strengths, weaknesses, avoid_terms, best_practices").eq("active", true).order("created_at", { ascending: false }).limit(12),
    supabase.from("creative_knowledge_blocks").select("id, title, domain, category, subcategory, definition, use_when, avoid_when, visual_effect, physical_description, prompt_fragment, avoid_constraints, compatible_output_types, compatible_platforms, compatible_industries, compatible_moods, required_inputs, missing_input_defaults, failure_risks, repair_notes, tags").eq("active", true).order("confidence_score", { ascending: false, nullsFirst: false }).limit(24)
  ]);
